...
//M.Yoshida
//バイパス用NNA_GenerateAuthEF
int
NNA_GenerateAuthEF_esd(NTP_Packet *packet, NTP_PacketInfo *info, SIV_Instance siv,
                   const unsigned char *nonce, int max_nonce_length,
                   const unsigned char *plaintext, int plaintext_length,
                   int min_ef_length)
{
...
//バイパス用変更点
  int tag_length = SIV_GetTagLength(siv);
  memset(ciphertext, 0, tag_length);
  unsigned char * cipher_data = ciphertext + tag_length;
  memcpy(cipher_data, plaintext, plaintext_length);
//  
  memset(ciphertext + ciphertext_length, 0, ciphertext_padding + additional_padding);
...
//M.Yoshida
//バイパス用NNA_DecryptAuthEF
int
NNA_DecryptAuthEF_esd(NTP_Packet *packet, NTP_PacketInfo *info, SIV_Instance siv, int ef_start,
                  unsigned char *plaintext, int buffer_length, int *plaintext_length)
{
  int siv_tag_length, max_siv_nonce_length, nonce_length, ciphertext_length;
  unsigned char *nonce, *ciphertext;
  ...
  *plaintext_length = ciphertext_length - siv_tag_length;
  assert(*plaintext_length >= 0);
//バイパス用変更点
  memcpy(plaintext, ciphertext + siv_tag_length, *plaintext_length);
//
  return 1;
}
