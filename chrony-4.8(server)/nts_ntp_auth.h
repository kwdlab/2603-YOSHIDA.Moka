...
//M.Yoshida
//バイパス用暗号化関数
extern int NNA_GenerateAuthEF_esd(NTP_Packet *packet, NTP_PacketInfo *info, SIV_Instance siv,
                              const unsigned char *nonce, int max_nonce_length,
                              const unsigned char *plaintext, int plaintext_length,
                              int min_ef_length);
...
//M.Yoshida
//バイパス用復号化関数
extern int NNA_DecryptAuthEF_esd(NTP_Packet *packet, NTP_PacketInfo *info, SIV_Instance siv,
                             int ef_start, unsigned char *plaintext, int buffer_length,
                             int *plaintext_length);
#endif
