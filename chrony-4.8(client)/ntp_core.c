...
//M.Yoshida//
  if(my_mode == MODE_CLIENT) {
          ...
          if (where_to->ip_addr.family == IPADDR_INET4) {
            struct in_addr print_addr;
            print_addr.s_addr = htonl(where_to->ip_addr.addr.in4);
            inet_ntop(AF_INET, &print_addr, ip_str, sizeof(ip_str));
          } else if (where_to->ip_addr.family == IPADDR_INET6) {
              inet_ntop(AF_INET6, where_to->ip_addr.addr.in6, ip_str, sizeof(ip_str));
          } else {
              snprintf(ip_str, sizeof(ip_str), "Unknown family (%u)", where_to->ip_addr.family);
          }

          printf("T1 = %ld.%09ld(to %s)\n", local_tx->ts.tv_sec, local_tx->ts.tv_nsec, ip_str);
  }
#endif
...
//M.Yoshida//
    if (inst->mode == MODE_CLIENT) {
        char ip_str[INET6_ADDRSTRLEN] = {0};
        if (inst->remote_addr.ip_addr.family == IPADDR_INET4) {
            struct in_addr addr;
            addr.s_addr = htonl(inst->remote_addr.ip_addr.addr.in4);
            inet_ntop(AF_INET, &addr, ip_str, sizeof(ip_str));
        } else if (inst->remote_addr.ip_addr.family == IPADDR_INET6) {
            inet_ntop(AF_INET6, inst->remote_addr.ip_addr.addr.in6, ip_str, sizeof(ip_str));
        } else {
            snprintf(ip_str, sizeof(ip_str), "Unknown family (%u)", inst->remote_addr.ip_addr.family);
        }

        uint32_t T2_hi = ntohl(message->receive_ts.hi);
        uint32_t T2_lo = ntohl(message->receive_ts.lo);

        double T2_ntp = (double)T2_hi + (double)T2_lo / 4294967296.0;
        double T2_unix = T2_ntp - 2208988800.0; // NTPエポック→UNIXエポック補正
        printf("T2 = %.9f(from %s)\n", T2_unix, ip_str);
    }
#endif
